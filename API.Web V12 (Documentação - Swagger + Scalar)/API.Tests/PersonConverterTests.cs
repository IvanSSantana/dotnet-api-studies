...
public class PersonConverterTests
{
    ...
    public void Parse_ShouldConvertPersonToPersonDTO()
    {
        ...
        result.Should()
            .BeEquivalentTo(dtoTest, options => options.Excluding(dto => dto.BirthDay));
    }
    ...
    public void ParseList_ShouldConvertListOfPersonToListOfPersonDTO()
    {
        ...
        result.Should().NotBeNull();
        result.Should().HaveCount(2);
        result[0].Should().BeEquivalentTo(dtoTest, options => options.Excluding(dto => dto.BirthDay));
        result[1].Should().BeEquivalentTo(dtoTest, options => options.Excluding(dto => dto.BirthDay));
    }
